...
class TitleScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (window.matchMedia && window.matchMedia('(max-device-width: 640px)').matches) {
            ...
            background.setInteractive();
            background.setOrigin(0, 0);
            this.add.text(D_WIDTH / 2, D_HEIGHT / 2, '横画面にしてください', { fontSize: '64px', color: '#fff', fontFamily: 'monospace' }).setFontSize(32).setOrigin(0.5);
            const fixScreen = this.add.text(D_WIDTH / 2, D_HEIGHT / 2 + 150, '横画面にしたらここをタップ', { fontSize: '64px', color: '#fff', fontFamily: 'monospace' }).setFontSize(16).setOrigin(0.5);
        ...
        this.matter.world.on('collisionstart', (event: Phaser.Physics.Matter.Events.CollisionStartEvent, bodyA: MatterJS.BodyType, bodyB: MatterJS.BodyType) => {
            event.pairs.forEach((pair) => {
                ...
                if (bodyA.gameObject && bodyB.gameObject && bodyA.gameObject.alpha === 0.25 && bodyB.gameObject.alpha === 0.25) {
                    ...
                    const restart = this.add.text(D_WIDTH / 2, D_HEIGHT / 2 + 100, 'Back to title', { fontSize: '16px', fontFamily: '"font"' }).setOrigin(0.5, 0.5);
                    // 画面のどこかをタップしたらゲーム画面へ
